...
actor CachedImageManager {
    ...
    func startCaching(for assets: [CameraPhotoAsset], targetSize: CGSize) {
    ...
    func stopCaching(for assets: [CameraPhotoAsset], targetSize: CGSize) {
        let phAssets = assets.compactMap { $0.phAsset }
        phAssets.forEach {
    ...
    @discardableResult
    func requestImage(for asset: CameraPhotoAsset, targetSize: CGSize, completion: @escaping ((image: Image?, isLowerQuality: Bool)?) -> Void) -> PHImageRequestID? {
